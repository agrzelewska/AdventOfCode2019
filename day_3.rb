class CrossedWires
  ...
  def count_manhattan_distance
    find_intersections

    distance = []
    @intersections.map { |i| distance << i.map(&:abs).reduce(:+) }

    distance.min
  end

  def minimize_signal_delay
    find_intersections

    routes = []
    @intersections.map do |cross|
      routes << [@route1.find_index(cross) + @route2.find_index(cross)]
    end

    routes.min.join.to_i + 2
  end

  private

  def find_intersections
    @route1 = find_route(input1)
    @route2 = find_route(input2)
  ...
  def find_route(input)
    ...
    input.split(',').map do |step|
      steps_count = step[1..-1].to_i
      steps = 1..steps_count

      case step[0]
      when "R"
        steps.each { |i| route << [row,col+i] }
        col += steps_count
      when "L"
        steps.each { |i| route<< [row,col-i] }
        col -= steps_count
      when "U"
        steps.each { |i| route << [row+i,col] }
        row += steps_count
      when "D"
        steps.each { |i| route << [row-i, col] }
        row -= steps_count
